...
const framer_package_versions_1 = require("./framer-package-versions");
...
function esbuildPluginBundleDependencies({ signal = undefined, externalPackages = [], externalizeNpm = false, outDir, onCollectMissingPackage = (pkg) => { }, }) {
        ...
        setup(build) {
            ...
            const resolveDep = async (args) => {
                ...
                if (externalPackages.some((x) => x === args.path || args.path.startsWith(x + '/'))) {
                    ...
                    const installed = await (0, exporter_1.resolvePackage)({
                        cwd: outDir,
                        pkg,
                    }).catch(() => '');
                    if (!installed) {
                        if (!reportedMissingPackages.has(pkg)) {
                            const hasFramerVersion = Object.prototype.hasOwnProperty.call(framer_package_versions_1.framerPackageVersions, pkg);
                            if (hasFramerVersion) {
                                const version = framer_package_versions_1.framerPackageVersions[pkg];
                                utils_1.spinner.info(`Missing package detected: ${pkg} (using Framer version ^${version})`);
                            }
                            else {
                                utils_1.spinner.info(`Missing package detected: ${pkg}`);
                            }
                            reportedMissingPackages.add(pkg);
                        }
                        // Check if we have a specific version from framerPackageVersions
                        const packageWithVersion = Object.prototype.hasOwnProperty.call(framer_package_versions_1.framerPackageVersions, pkg)
                            ? `${pkg}@^${framer_package_versions_1.framerPackageVersions[pkg]}`
                            : pkg;
